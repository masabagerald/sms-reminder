...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.infant.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.infants.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('mother') ? 'is-invalid' : '' }}" name="mother_id" id="mother_id" required>
                    @foreach($mothers as $id => $mother)
                        <option value="{{ $id }}" {{ old('mother_id') == $id ? 'selected' : '' }}>{{ $mother }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('eid_no') ? 'is-invalid' : '' }}" type="text" name="eid_no" id="eid_no" value="{{ old('eid_no', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('pcr_exp_date') ? 'is-invalid' : '' }}" type="text" name="pcr_exp_date" id="pcr_exp_date" value="{{ old('pcr_exp_date') }}">
